...
def main():
    ...
    with open('results.csv','w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(["Hostname", "Any ARP Records", "Any Active NAT Stmts", "Firepower Present", "External IP", "Internal IP"])
...
def decom_checks(net_connect,current_fw,external_ip,internal_ip):
    ...
    fire_power_check = get_firepower_status(net_connect)
    results.append([current_fw,arp_check,nat_check,fire_power_check,external_ip,internal_ip,])
    ##Output results to a CSV file
    output_to_csv(results)

##Writes results of the script to a CSV file
def output_to_csv(results):
    with open('results.csv','a', newline='') as f:
        writer = csv.writer(f)
        writer.writerows(results)

##Checks modules on the firewall and if runnning firepower this alerts the user
## and this can be later deactivated 
def get_firepower_status(net_connect):
    try:
        firepower_status = net_connect.send_command('show module | i sfr')
    except:
        return("Cannot Access")

    for line in firepower_status.split('\n'):
        if 'sfr' in line:
            return True
            break
        else:
...
##Grabbing ARP records from the firewall
...
##Checks the client firewall is actually online
